...
import {Button, Select, Text, TreeSelect} from "../../../components/utils/field";
...
import {createTree} from "../../../utils/funcTree";
import {getBaseMenuByParent, getMenuByParentParentId, getLang} from "../../../services/getList";
...
function List() {
    ...
    const columns = [
        {
            title: 'ردیف',
            key: 'index',
            render : (text, record, index) => ((page*pageSize)-pageSize)+index+1
        },
        {
            title: 'عنوان',
            key: 'Title',
        ...
        },
        {
            ...
            className:'lan-col',
            render:(field,record)=>(
                <div className='lang-part d-flex align-items-center justify-content-center'>
                    {field.map((item)=>{
                            let flag=setFlag(item)
            ...
            render: (f,r) => (
                <div className='action-part' style={{width:'auto',justifyContent:"center"}}>
                    ...
                    <Tooltip title="ویرایش">
                        <button className='action-btn edit-btn' onClick={()=>openEdit(r?.Id,r?.ParentId)}>
    ...
    function removeItem() {
        ...
        axios.delete(url,apiData)
            .then(res=> {
                message.success('عملیات با موفقیت انجام شد.')
                getData()
            })
            .catch(err=> {
                err?.response?.data?.message?message.error(err?.response?.data?.message):message.error('مشکلی رخ داده است.')
                setLoading(false)
            })
    ...
    const onFinish = useCallback((values) => {
        ...
        if(isEmpty&&!isClear){
            ...
            setFormValues({
                type:isNaN(values.type)?'':parseInt(values.type,10),
                title:values?.title,
                parentId: isNaN(values.parentId)?'':parseInt(values.parentId,10),
            })
            setPage(1)
            setIsClear(false)
        }
    ...
    function getData(){
        setLoading(true)
        if(langId===1){
            axios.get(`Menu/GetAllBaseMenu?Page=${
                page}&PageSize=${pageSize}&Title=${formValues.title||''}&ParentId=${formValues.parentId||''}${formValues.type?`&TypeId=${formValues.type}`:''}`)
                .then(res=> {
            ...
            axios.get(`Menu/GetAllMenu?Page=${
                page}&PageSize=${pageSize}&Title=${formValues.title||''}&ParentId=${formValues.parentId||''}&Type=${formValues.type||''
            }&LanguageId=${langId||''}`)
                .then(res=> {
                    if(res?.data?.data?.length===0||!Object.keys(res?.data?.data).length){
                        notFound()
    ...
    return(
        <div className='page-content'>
            <Helmet><title>وب سایت-فهرست</title></Helmet>
            ...
            >
                <div className='form-search-row'>
                    ...
                    <div className='form-search-col position-relative'>
                        {loadParent&&<div style={{position:'absolute',inset:'0',height:'43px',background:'rgba(233,236,246,.11)',zIndex:'5', display:'flex',alignItems:'center',justifyContent:'center'}}><Spin/></div>}
                        <TreeSelect
                            label='عنوان اصلی'
                            name='parentId'
                            allowClear
            ...
            >
                ...
                <p>
                    شما در حال حذف کردن یک آیتم از منو هستید
                    <br/>
                    آیا از این کار مطمئنید؟
                </p>
            </Modal>
            {editModal&&<EditMenu closeModal={closeModal} getData={getData} editRecord={editRecord} />}
        </div>
    )
}
